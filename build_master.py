...
#     (b) 204件群（row[8] が row[1] を先頭以外で含む＝頭に余分な語が付いている型）から 3行
...
TITLE_KANA_OVERRIDE = {
    ...
    # ======== (b) 204件群（row[8] が row[1] を先頭以外で含む型）からの3行（2026-08-05） ========
    #
    # 上の54行と群が違う。上は row[8] が row[1] の**接頭辞**（末尾に副題が付いている）だが、
    # この3行は row[8] の**頭に余分な語が付いていて** row[1] をその後ろに含む型。
    ...
    #   3行とも現行の row[8] では **0件**（HTTP 404 /「該当件数：0件」）。頭の語が余分で
    #   駿河屋のカタログに1件も当たらない。row[1] に落ちると3行とも1ページ目に目的の商品が
    #   出る（1位1行 / 2位1行 / 6位1行）。0件 → 到達が 3/3。
    #
    # ★204件群への一律修正ではない。n=3 の名指しであって、群全体（204行）には広げていない
...
def main(ttl_path, out_path, wd_path=None, pub_path=None, ja_path=None):
    ...
    # 調査時 57行（145件群から54 + 204件群から3。いずれも罠#95 の到達確認済み）。
