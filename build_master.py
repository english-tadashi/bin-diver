...
WIKIDATA_MISLINK = {
    ...
    # --- 2026-08-17（続き）year 差の「MADB年が先」20件から2件 ---
    #   ★year 差は絞り込みに使えないことが確定した。403件のうち383件は
    #     「MADB年が後」＝移植・再録で正常（アーケードアーカイブス +34年 等）。
    #     「MADB年が先」20件も、17件は Wikidata の MIN(P577) が後発リリースを
    #     拾っているだけだった（光栄6件が同一 wd 2001 に揃うなど）。
    #   ★M726986（ぽけっとぷよぷよ通）はここに入れない。リンクは妥当で、
    #     誤っているのは MADB の日付（1991-07-26 と DMG-APYJ-JPN が矛盾。
    #     GB の4文字コードは90年代半ば以降の採番）。この表では扱えない。
    #   ★2件とも列集合は {"en","kana"}。en も Wikidata 由来で汚染側の題名が来ている
    #     （wikidata_en.json を直接引いて確認した）。
    #   ★棚初字は2件とも無傷（@ja-hrkt が仮名で第1の源が埋まる）。下の「見込み 5」は変わらない。
    #     M727800 … @ja-hrkt「ハイパー オリンピック」→ ハ
    #     M755396 … @ja-hrkt「プヨプヨ」→ フ
    "M727800": ({"en", "kana"}, "Track & Field II",                   "ハイパーオリンピック (FC 1985-06-21・コナミ/RC800)。kana=科樂美ックスポーツ イン ソウル・en=Track & Field II はどちらも3年後の続編(wd 1988)を指す。MADB 側は ja・@ja-hrkt・型番まで整合しており無傷"),
    "M755396": ({"en", "kana"}, "Puyo Puyo!! 20th Anniversary",       "ぷよぷよ!! スペシャルプライス (DS 2008-06-19・セガ)。リンク先 ぷよぷよ!! 20th anniversary は wd 2011 で、20周年版が2008年に出ることはない。@ja-hrkt が「プヨプヨ」しか無く照合の手がかりが弱い行"),
    # --- ② 同一シリーズの別機種版の題名（2件）---
    ...
    # --- 2026-08-17（続き）コナミのスポーツ系の鏡像 ---
    #   M727800（ハイパーオリンピック FC 1985）と互いの題名を持っていた鏡像。
    #   kana = ハイパーオリンピック は 1983年アーケード版の名で、
    #   M727800 の title_ja と完全一致する。リンク先は wd year 1983・pfids 8機種
    #   ＝アーケード項目。
    #   ★en = Track & Field は残す。出所はアーケード項目だが、GB版の実際の
    #     海外題も Track & Field なので、この行そのものに当たる。
    #     M756532（Bomberman 64）とは結論が逆になる ―― あちらは1997年版へ
    #     検索が流れるので止めたが、こちらは流れない。
    #   ★棚初字は無傷（@ja-hrkt 'コナミック スポーツ イン バルセロナ' が仮名で第1の源が埋まる）。
    #     現在の kana_row は コ で、塞いでも変わらない。下の「見込み 5」も変わらない。
    "M748331": ({"kana"}, "", "コナミックスポーツ イン バルセロナ (GB 1992) ← kana だけ ハイパーオリンピック"),
    # ★M878083（コナミックスポーツ イン ソウル FC 1988）は英題なしのまま残す。
    #   Track & Field II は本来この行に付くべき値だが、入れる経路が無い:
    #     MADB … schema:description が無い（TITLE_EN の6件は全て description
    #             に英語があり、字面を整えただけ。外から値を持ち込んでいない）
    #     Wikidata … 該当項目の P7886 は M727800 を指しており、この行には
    #             1本も貼られていない（pfids も wd year も空）
    #   人が転記すれば入るが、それは罠#141 と M727424 の「英題を作らない」に抵触する。
    #   duck hunt の保留（ノート 9156 / 9293行）と同じ棚。
    # --- ★両列とも汚染（1件）---
